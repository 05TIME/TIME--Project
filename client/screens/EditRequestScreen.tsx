...
type RouteType = RouteProp<RootStackParamList, "EditRequest">;
...
export default function EditRequestScreen() {
  ...
  const existingEndpoint = route.params?.endpoint;

  const [method, setMethod] = useState<HttpMethod>(existingEndpoint?.method || "GET");
  const [url, setUrl] = useState(existingEndpoint?.url || "");
  const [name, setName] = useState(existingEndpoint?.name || "");
  const [headers, setHeaders] = useState(
    existingEndpoint?.headers ? JSON.stringify(existingEndpoint.headers, null, 2) : ""
  );
  const [body, setBody] = useState(existingEndpoint?.body || "");
  ...
  const sendRequest = async () => {
    ...
    try {
      ...
      const endpoint: Endpoint = {
        id: existingEndpoint?.id || Date.now().toString(),
        ...
        lastUsed: Date.now(),
        createdAt: existingEndpoint?.createdAt || Date.now(),
      };
